...
logging.info(
    f"Enabled data types: incidents({config.data_types.incidents.enabled}), alerts({config.data_types.alerts.enabled}), schedules({config.data_types.schedules.enabled}), escalation_policies({config.data_types.escalation_policies.enabled})"
)
...
def log_request_details(request: httpx.Request):
    request.read()
    logging.debug("--- HTTPX Request Details (Event Hook) ---")
    ...
    logging.debug("--- End HTTPX Request Details (Event Hook) ---")
...
if __name__ == "__main__":
    ...
    if since:
        try:
            ...
            logging.error(
                f"Invalid 'since' date format: {since}. Error: {e}. Please use ISO-8601 format (e.g., YYYY-MM-DDTHH:MM:SSZ)."
            )
            sys.exit(1)
    else:
        logging.info("No 'since' date provided; fetching all enabled data types with configured limits.")
    ...
    try:
        with glean_client() as c:
            ...
            try:
                index_api_response = c.indexing.documents.index(
                    datasource=config.glean.datasource_name, documents=all_documents
                )
        ...
        logging.error(
            f"HTTP error occurred with Rootly: {http_err} - Response: {http_err.response.text if http_err.response else 'No response body'}"
        )
        sys.exit(1)
